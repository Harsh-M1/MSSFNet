...
class MobileViTBlock(nn.Module):
    ...
    def folding(self, x: Tensor, info_dict: Dict) -> Tensor:
        ...
        x = x.reshape(batch_size, channels, num_patch_h *
                      self.patch_h, num_patch_w * self.patch_w)
...
class MobileViT(nn.Module):
    ...
    def __init__(self, model_cfg: Dict, num_classes: int = 1000):
        ...
        # self.classifier.add_module(name="fc", module=nn.Linear(
        #     in_features=exp_channels, out_features=num_classes))
...
def mobile_vit_small(num_classes: int = 1000, pretrained=False):
    # pretrain weight link
    # https://docs-assets.developer.apple.com/ml-research/models/cvnets/classification/mobilevit_s.pt
    config = get_config("small")
    m = MobileViT(config, num_classes=num_classes)
    if pretrained:
        m.load_state_dict(model_zoo.load_url(
            'https://docs-assets.developer.apple.com/ml-research/models/cvnets/classification/mobilevit_s.pt', model_dir='model_data'), strict=False)
...
if __name__ == '__main__':
    ...
    # vit = mobile_vit_xx_small()
    # out = vit(img)
    # print(out.shape)
    # print(count_parameters(vit))

    # vit = mobile_vit_x_small()
    # out = vit(img)
    # print(out.shape)
    # print(count_parameters(vit))

    vit = mobile_vit_small()
    out = vit(img)
    print(out[0].shape)
    print(out[1].shape)
    print(out[2].shape)
