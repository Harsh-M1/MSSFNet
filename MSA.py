...
class LayerNorm2d(nn.LayerNorm):
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out = out / \
            torch.sqrt(torch.square(out).mean(dim=1, keepdim=True) + self.eps)
...
class ConvLayer(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, kernel_size=3, stride=1, dilation=1, groups=1,
        ...
        self.dropout = nn.Dropout2d(
            dropout, inplace=False) if dropout > 0 else None
...
class MSABlock(nn.Module):
    r"""Multiscale Spatial Attention Block"""
    ...
    def relu_linear_att(self, qkv: torch.Tensor) -> torch.Tensor:
        ...
        q, k, v = (qkv[..., 0: self.dim], qkv[...,
                   self.dim: 2 * self.dim], qkv[..., 2 * self.dim:],)
